...
lstsold::block.smooth.periodogram(y, spar.freq = 0.8, spar.time = 0.8, theta = 90, phi = 0)
lsts::lsts_spb (y, spar_freq = 0.8, spar_time = 0.8, theta = 90, phi = 0)
...
for(j in 1:M){
  x = y[(1 + S*(j-1)):(N + S*(j-1))]
  table = rbind(table,nlminb(start = c(0.65, 0.15), N = N, objective = lstsold::LS.whittle.loglik,
...
fit = lstsold::LS.whittle(series = y, start = c(coef(fit.1), coef(fit.2)), order = c(p = 1, q = 0),
                 ar.order = c(1), sd.order = 1, N = 180, n.ahead = 10)

## Summary
lstsold::LS.summary(fit)

fit = lsts::lsts_we(series = y, start = c(coef(fit.1), coef(fit.2)), order = c(p = 1, q = 0),
                          ar.order = c(1), sd.order = 1, N = 180, n.ahead = 10)
