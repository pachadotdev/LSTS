...
malleco = read.table("dev/malleco.txt", header = T) 
...
smooth_periodogram_old = lstsold::block.smooth.periodogram(y, spar.freq = 0.8, spar.time = 0.8, theta = 90, phi = 0)
smooth_periodogram_new = lsts::lsts_spb (y, spar_freq = 0.8, spar_time = 0.8, theta = 90, phi = 0)

all.equal(smooth_periodogram_old, smooth_periodogram_new)
...
for(j in 1:M){
  x = y[(1 + S*(j-1)):(N + S*(j-1))]
  table = rbind(table,nlminb(start = c(0.65, 0.15), N = N,
                             objective = lstsold::LS.whittle.loglik,
...
ls_whittle_old = lstsold::LS.whittle(series = y, start = c(coef(fit.1), coef(fit.2)), order = c(p = 1, q = 0),
                 ar.order = c(1), sd.order = 1, N = 180, n.ahead = 10)

## Summary
lstsold::LS.summary(ls_whittle_old)

fit2 = lsts::lsts_we(series = y, 
                     start = c(coef(fit.1), coef(fit.2)),
                     order = c(p = 1, q = 0),
                     ar.order = c(1),
                     sd.order = 1,
                     N = 180,
...
## Diagnostic
lstsold::ts.diag(ls_whittle_old$residuals)
...
for(alpha in seq(0.05,1.00, 0.05)){
  LI = fit$pred - fit$se * qnorm(1-alpha/2)
  LS	= fit$pred + fit$se * qnorm(1-alpha/2)
  polygon(c(LS,rev(LI))~c(time(LS), rev(time(LI))), col = gray(1-alpha), border = gray(1-alpha))	
}