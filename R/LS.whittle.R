...
#'
#' @description This function computes Whittle estimator to LS-ARMA and LS-ARFIMA models.
#'
#' @details
...
#'
#' @param include.d (type: numeric) logical argument for \code{ARFIMA} models. If \code{include.d=FALSE} then the model is an ARMA process.
#'
#' @param N (type: numeric) value corresponding to the length of the window to compute periodogram. If \code{N=NULL} then the function will use \eqn{N = \textmd{trunc}(n^{0.8})}, see Dahlhaus (1998) where \eqn{n} is the length of the \code{y} vector.
#'
#' @param S (type: numeric) value corresponding to the lag with which will go taking the blocks or windows.
#'
#' @param include.taper (type: logical) logical argument that by default is \code{TRUE}. See \code{\link{periodogram}}.
#'
#' @param control (type: list) A list of control parameters. More details in \code{\link[stats]{nlminb}} .
#'
#' @param lower (type: numeric) lower bound, replicated to be as long as \code{start}. If unspecified, all parameters are assumed to be lower unconstrained.
#'
#' @param upper (type: numeric) upper bound, replicated to be as long as \code{start}. If unspecified, all parameters are assumed to be upper unconstrained.
#'
#' @param m (type: numeric) truncation order of the MA infinity process, by default \eqn{m = 0.25*n^{0.8}}. Parameter used in \code{LS.kalman}.
#'
#' @param n.ahead (type: numeric) The number of steps ahead for which prediction is required. By default is zero.
#'
#' @examples
