...
#'
#' @description This function computes the periodogram from a stationary time serie. Returns
#' the periodogram, its graph and the Fourier frequency.
#'
#' @details
#' The tapered periodogram it is given by \deqn{I(\lambda) = \frac{|D_n(\lambda)|^2}{2\pi
#' H_{2,n}(0)}} with \eqn{D(\lambda) = \sum_{s=0}^{n-1} h\left(\frac{s}{N}\right) y_{s+1}\,
#' e^{-i\,\lambda\,s}}, \eqn{H_{k,n} = \sum_{s=0}^{n-1}h\left(\frac{s}{N}\right)^k\,
#' e^{-i\,\lambda\,s}} and \eqn{\lambda} are Fourier frequencies defined as \eqn{2\pi k/n},
#' with \eqn{k = 1,\,\ldots,\, n}.
#'
#' The data taper used is the cosine bell function, \eqn{h(x) = \frac{1}{2}[1-cos(2\pi x)]}.
#' If the series has missing data, these are replaced by the average of the data and \eqn{n}
#' it is corrected by $n-N$, where \eqn{N} is the amount of missing values of serie.
...
#'
#' @param include.taper (type: logical) logical argument which by default is \code{FALSE}.
#' If \code{include.taper=TRUE} then \code{y} is multiplied by \eqn{0.5*(1 - cos(2\pi(n-1)/n))
#' (\emph{cosine bell})}.
#'
#' @references
