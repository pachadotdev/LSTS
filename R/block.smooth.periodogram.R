...
#'
#' @description Plots in 3D the smoothing periodogram of a time series, by blocks or windows.
#'
#' @details
#' The number of windows of the function is \eqn{M = \textmd{trunc}((n-N)/S+1)}, where
#' \code{\link[base]{trunc}} truncates de entered value and \emph{n} is the length of the vector
#' \code{y}. All windows are of the same length \code{N}, if this value isn't entered by user then
#' is computed as \eqn{N=\textmd{trunc}(n^{0.8})} (Dahlhaus).
#'
#' \code{block.smooth.periodogram} computes the periodogram in each of the \emph{M} windows and then
#' smoothes it two times with \code{\link[stats]{smooth.spline}} function; the first time using
#' \code{spar.freq} parameter and the second time with \code{spar.time}. These windows overlap between
#' them.
#'
#' The surface is then viewed by looking at the origin from a direction defined by \code{theta} and
#' \code{phi}. If \code{theta} and \code{phi} are both zero the viewing direction is directly down
#' the negative y axis. Changing \code{theta} will vary the azimuth and changing \code{phi}
#' the colatitude.
#'
...
#'
#' @param ylab (type: character) title for the y-axis. By default is \code{ylab="Time"} and is an optional
#' parameter.
#'
